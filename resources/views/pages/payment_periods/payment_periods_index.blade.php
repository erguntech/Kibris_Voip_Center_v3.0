...
@section('PageTitle', __('messages.payment_periods.index.01'))
        ...
        <div class="col-xl-12">
            <div class="alert alert-info d-flex align-items-center p-5">
                ...
                <div class="d-flex flex-column">
                    ...
                    <span>- {{ __('messages.payment_periods.alert.03') }}</span>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    ...
                    <div class="card-toolbar">
                        <a href="{{ route('PaymentPeriods.Create') }}" class="btn btn-light-success btn-icon btn-sm">
                ...
                <div class="card-body">
                    <div class="row p-0">
                        ...
                        <div class="col">
                            <div class="border border-dashed border-gray-300 text-center min-w-125px rounded pt-4 pb-2">
                                <span class="fs-4 fw-semibold text-success d-block">{{ __('messages.payment_periods.index.03') }}</span>
                                <span class="fs-2hx fw-bold text-gray-900">{{ $clientCount }}</span>
                            </div>
                        </div>
                        <!--end::Col-->
                        <!--begin::Col-->
                        <div class="col">
                            <div class="border border-dashed border-gray-300 text-center min-w-125px rounded pt-4 pb-2">
                                <span class="fs-4 fw-semibold text-danger d-block">{{ __('messages.payment_periods.index.04') }}</span>
                                <span class="fs-2hx fw-bold text-gray-900">{{ $totalSummaryReturn }}</span>
                            </div>
                        </div>
                        <!--end::Col-->
                    </div>
                    ...
                    <table id="datatable" class="table table-hover table-rounded table-row-bordered table-row-gray-200 gy-1 gs-10" style="min-height: 100%; width: 100% !important;">
                        ...
                        <tr class="fw-bolder fs-7 text-uppercase gy-5">
                            ...
                            <th>{{ __('messages.payment_periods.index.datatables.01') }}</th>
                            <th>{{ __('messages.payment_periods.index.datatables.02') }}</th>
                            <th>{{ __('messages.payment_periods.index.datatables.03') }}</th>
                            <th>{{ __('messages.payment_periods.index.datatables.04') }}</th>
                            <th>{{ __('messages.payment_periods.index.datatables.07') }}</th>
...
@section('PageCustomJS')
    <script>
        var table, dt;
        var editUrl = '{{ route("PaymentPeriods.Edit", ":id") }}';
        ...
        var initDatatable = function () {
            dt = $("#datatable").DataTable({
                ...
                info: false,
                ajax: {
                    url : "{{ route('PaymentPeriods.Index') }}",
                },
                language: {
                    @if(App::getLocale() == 'tr')
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "paymentperiods/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {
