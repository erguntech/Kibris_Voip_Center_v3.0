...
@section('PageTitle', __('messages.payment_periods.edit.01'))
        ...
        <div class="col-xl-12">
            ...
            <div class="card shadow-sm">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;"><span class="d-flex text-white fw-bolder fs-6">{{ __('messages.payment_periods.edit.02') }}</span></div>
                    <h3 class="card-title"></h3>
                    <div class="card-toolbar"></div>
                </div>
                <div class="card-body">
                    <form action="{{ route('PaymentPeriods.Update', $paymentPeriod->id) }}" id="editForm" enctype="multipart/form-data" method="POST" autocomplete="off">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-12">
                                <label for="input-client_id" class="required form-label">{{ __('messages.payment_periods.form.01') }}</label>
                                <select class="form-select @error('input-client_id') is-invalid error-input @enderror" name="input-client_id" id="input-client_id" disabled>
                                    <option value="{{ $paymentPeriod->client_id }}">{{ $paymentPeriod->linkedClient->company_name }}</option>
                        ...
                        <div class="row mt-6">
                            <div class="col-6">
                                ...
                                <input type="text" name="input-payment_amount" id="input-payment_amount" class="form-control @error('input-payment_amount') is-invalid error-input @enderror" placeholder="{{ __('messages.payment_periods.form.03') }} {{ __('messages.forms.01') }}" maxlength="150" value="{{ old('input-payment_amount', $paymentPeriod->payment_amount) }}"/>
                            ...
                            <div class="col-6">
                                ...
                                <select class="form-select @error('input-currency') is-invalid error-input @enderror" name="input-currency" id="input-currency" data-control="select2" data-placeholder="{{ __('messages.forms.06') }}" data-hide-search="true">
                                    <option></option>
                                    <option value="TRY" {{ old('input-currency', $paymentPeriod->currency) == 'TRY' ? 'selected' : '' }}>{{ __('messages.payment_periods.form.04.01') }}</option>
                                    <option value="USD" {{ old('input-currency', $paymentPeriod->currency) == 'USD' ? 'selected' : '' }}>{{ __('messages.payment_periods.form.04.02') }}</option>
                                    <option value="EUR" {{ old('input-currency', $paymentPeriod->currency) == 'EUR' ? 'selected' : '' }}>{{ __('messages.payment_periods.form.04.03') }}</option>
                                </select>
                        ...
                        <div class="row mt-6">
                            <div class="col-6">
                                ...
                                <input name="input-payment_start_date" id="input-payment_start_date" class="form-control @error('input-payment_start_date') is-invalid error-input @enderror" placeholder="{{ __('messages.forms.06') }}" value="{{ old('input-payment_start_date', $paymentPeriod->payment_start_date) }}"/>
                            ...
                            <div class="col-6">
                                ...
                                <select class="form-select @error('input-show_delayed_payment_warnings') is-invalid error-input @enderror" name="input-show_delayed_payment_warnings" id="input-show_delayed_payment_warnings" data-control="select2" data-placeholder="{{ __('messages.forms.06') }}" data-hide-search="true">
                                    <option></option>
                                    <option value="1" {{ old('input-show_delayed_payment_warnings', $paymentPeriod->show_delayed_payment_warnings) == '1' ? 'selected' : '' }}>{{ __('messages.payment_periods.form.05.01') }}</option>
                                    <option value="0" {{ old('input-show_delayed_payment_warnings', $paymentPeriod->show_delayed_payment_warnings) == '0' ? 'selected' : '' }}>{{ __('messages.payment_periods.form.05.02') }}</option>
                                </select>
