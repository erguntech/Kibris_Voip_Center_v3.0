...
@section('PageTitle', __('messages.sms_devices.index.01'))
            ...
            <div class="card">
                <div class="card-header ribbon ribbon-start ribbon-clip">
                    ...
                    <div class="card-toolbar">
                        <a href="{{ route('SMSDevices.Create') }}" class="btn btn-light-success btn-icon btn-sm">
                    ...
                    <div class="row p-0">
                        ...
                        <table id="datatable" class="table table-hover table-rounded table-row-bordered table-row-gray-200 gy-1 gs-10" style="min-height: 100%; width: 100% !important;">
                            ...
                            <tr class="fw-bolder fs-7 text-uppercase gy-5">
                                ...
                                <th>{{ __('messages.sms_devices.index.datatables.01') }}</th>
                                <th>{{ __('messages.sms_devices.index.datatables.02') }}</th>
                                <th>{{ __('messages.sms_devices.index.datatables.03') }}</th>
                                <th>{{ __('messages.sms_devices.index.datatables.04') }}</th>
                                <th>{{ __('messages.sms_devices.index.datatables.05') }}</th>
...
@section('PageCustomJS')
    <script>
        var table, dt;
        var editUrl = '{{ route("SMSDevices.Edit", ":id") }}';
            ...
            dt = $("#datatable").DataTable({
                ...
                columnDefs: [
                    ...
                    { targets: -1,
                        ...
                        render: function (data, type, row) {
                            return `
                            <a class="btn btn-icon btn-light-primary" onclick="assignCredits(this)" data-id="`+ row.id +`" style="height: calc(2.05em);"><i class="fas fa-comment-sms fs-5" style="margin-top: 2px;"></i></a>
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "smsdevices/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {
    ...
    <script type="text/javascript">
        ...
        function assignCredits(btn) {
            Swal.fire({
                title: '{{ __('messages.sweetalert.38') }}',
                html:
                    `<div class="col-12">
                        <input type="text" name="input-credits_to_add" id="input-credits_to_add" class="form-control" placeholder="{{ __('messages.sms_credits.form.02') }} {{ __('messages.forms.01') }}" maxlength="50" value="{{ old('input-credits_to_add') }}"/>
                    </div>`,
                icon: 'success',
                showCancelButton: true,
                allowOutsideClick: false,
                ...
                if(result) {
                    if ($('#input-credits_to_add').val() == "" || !containsOnlyDigits($('#input-credits_to_add').val())) {
                        Swal.fire({
                            title: '{{ __('messages.sweetalert.42') }}',
                            text: '{{ __('messages.sweetalert.43') }}',
                            icon: 'error',
                            showCancelButton: true,
                        ...
                        if (result.value) {
                            $.ajax({
                                url: 'ajax/devicesmscredits',
                                type: 'POST',
                                data: {
                                    "deviceID": btn.getAttribute('data-id'),
                                ...
                                success: function (data){
                                    Swal.fire({
                                        icon: 'success',
                                        title: '{{ __('messages.sweetalert.39') }}',
                                        html: '<br>{{ __('messages.sweetalert.40') }} <br><br> <strong class="text-warning">{{ __('messages.sweetalert.29') }}:</strong> '+ data.oldCredits.toLocaleString('tr-TR') +'<br><strong class="text-primary">{{ __('messages.sweetalert.30') }}: </strong>'+ data.creditsToAdd.toLocaleString('tr-TR') +'<br><strong class="text-success">{{ __('messages.sweetalert.31') }}: </strong>'+ data.totalCredits.toLocaleString('tr-TR') +'<br><br>{{ __('messages.sweetalert.13') }}',
                                        confirmButtonText: '{{ __('messages.sweetalert.05') }}',
                                        allowOutsideClick: false,
                                        customClass: {
                                            confirmButton: 'btn btn-success',
