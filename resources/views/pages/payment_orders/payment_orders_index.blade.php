...
@section('PageTitle', __('messages.payment_orders.index.01'))
                ...
                <div class="card-body">
                    ...
                    <table id="datatable" class="table table-hover table-rounded table-row-bordered table-row-gray-200 gy-1 gs-10" style="min-height: 100%; width: 100% !important;">
                        ...
                        <tr class="fw-bolder fs-7 text-uppercase gy-5">
                            ...
                            <th>{{ __('messages.payment_orders.index.datatables.01') }}</th>
                            <th>{{ __('messages.payment_orders.index.datatables.02') }}</th>
                            <th>{{ __('messages.payment_orders.index.datatables.03') }}</th>
                            <th>{{ __('messages.payment_orders.index.datatables.04') }}</th>
                            <th>{{ __('messages.payment_orders.index.datatables.05') }}</th>
...
@section('PageCustomJS')
    <script>
        var table, dt;
        var editUrl = '{{ route("PaymentOrders.Edit", ":id") }}';
                ...
                columnDefs: [
                    ...
                    { targets: -1,
                        ...
                        render: function (data, type, row) {
                            if(row.status != 1) {
                                return `
                                <a class="btn btn-icon btn-light-success" onclick="approvePayment(this)" data-id="`+ row.id +`" style="height: calc(2.05em);"><i class="fas fa-thumbs-up fs-5" style="margin-top: 1px;"></i></a>
                                <a class="btn btn-icon btn-light-primary" href="`+ detailUrl.replace(':id', row.id) +`" style="height: calc(2.05em);"><i class="fas fa-eye fs-5" style="margin-top: 1px;"></i></a>
                                `;
                            } else {
    ...
    <script type="text/javascript">
        function deleteData(btn) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "paymentorders/"+btn.getAttribute('data-id'),
                        type: 'DELETE',
                        data: {
    ...
    <script type="text/javascript">
        function approvePayment(btn) {
            Swal.fire({
                title: '{{ __('messages.sweetalert.16') }}',
                text: '{{ __('messages.sweetalert.17') }}',
                icon: 'success',
                showCancelButton: true,
                allowOutsideClick: false,
                confirmButtonText: '{{ __('messages.sweetalert.03') }}!',
